...
try:
    ...
    #FILTERED SEARCH
    rawisfiltered = input('\nWould you like to preform an filtered search? (Y/N) --> ')
    if rawisfiltered == 'Y' or rawisfiltered == 'y':
        isfiltered = True
        print('Please input the package you would like to search for in a comma seperated list.')
        print('Example: APA150-TQG100 would become APA150 and these packages would be input as "APA150, APA075,..., APA450"')
        rawpackages = input('packages --> ').replace(' ', '')
        packages = rawpackages.split(',')
        print('\nNow, please input the part you would like to search for in a comma seperated list.')
        print('Example: APA150-TQG100 would become TQG100 and these parts would be input as "TQG100, BG456L,..., BG456M"')
        rawparts = input('parts --> ').replace(' ', '')
        parts = rawparts.split(',')
        qualifiers = packages.copy()
    ...
    for i in range(ndevices):                                                   #Iterate through list of devices on findchips.com
        ...
        if(isfiltered):                                                         #Only query if the part number is wanted.
            for qualifier in qualifiers:
                if qualifier in partnum:
                    goodpartnum = True
                    break
                else:
                    goodpartnum = False
        ...
        print(f'{str(a)} ---row-{str(i + 2)}---> {str(url)}')                  #Print loop iteration and URL being scraped
